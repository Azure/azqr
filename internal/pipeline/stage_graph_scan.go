...
func (s *GraphScanStage) CanSkip(ctx *ScanContext) bool {
	shouldSkip := !ctx.Params.UseGraphRecommendations
	log.Debug().
		Bool("use_graph", ctx.Params.UseGraphRecommendations).
		Bool("will_skip", shouldSkip).
		Msg("Graph stage skip check")
	return shouldSkip
}
