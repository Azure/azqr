...
func BuildTools() []copilot.Tool {
	return []copilot.Tool{
		copilot.DefineTool("scan", `Run Azure Quick Review scan to analyze Azure resources for compliance, security, and best practices.

Stages enabled by default: graph, diagnostics, advisor, defender.
Stages disabled by default (must be explicitly named to enable): policy, defender-recommendations, cost, arc.

To enable optional stages pass their names in the "stages" array:
  - {"stages": ["policy", "defender-recommendations"]}  -> enable Policy and Defender Recommendations
  - {"stages": ["policy", "defender-recommendations", "+cost"]}  -> also enable Cost
  - {"stages": ["-advisor"]}  -> disable Advisor only (run defaults minus advisor)
Use "-" prefix to disable a default stage; bare name or "+" prefix to enable an optional stage.`,
		...
		func() copilot.Tool {
			t := copilot.DefineTool("get-recommendations-catalog", "Get the complete catalog of azqr recommendations",
		...
		func() copilot.Tool {
			t := copilot.DefineTool("get-supported-services", "List all Azure services supported by azqr",
				func(_ emptyParams, _ copilot.ToolInvocation) (string, error) {
					ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
					defer cancel()
