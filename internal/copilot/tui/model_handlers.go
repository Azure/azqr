...
func (m *Model) formatReasoning(text string) string {
	...
	for _, line := range strings.Split(WrapText(text, m.width), "\n") {
		...
		if len(lines) == 0 {
			lines = append(lines, m.styles.ReasoningText.Render("● ")+m.styles.ReasoningText.Render(line))
		} else {
			lines = append(lines, m.styles.ReasoningText.Render("  "+line))
		}
	}
	return strings.Join(lines, "\n")
}

// formatAssistant formats assistant text for output history, rendering markdown.
...
func (m *Model) resumeMsg() string {
	...
	return "Resume this session with " + cmd + "\n"
}
