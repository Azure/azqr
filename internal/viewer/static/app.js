...
const menuStructure = [
    { label: 'Home', route: 'dashboard', children: [] },
    {
...
function getMenuIcon(label) {
    const icons = {
        'Home': '<i class="bi bi-house-door me-1"></i>',
...
async function showDataset(name) {
    ...
    document.getElementById('dataset-title').innerText = routeLabels[name] || name;
...
function renderTable(rows, datasetName) {
    ...
    html += finalHeaders.map((field, index) => {
        ...
        const filterConfig = {
            ...
            'defender': ['subscriptionId', 'subscriptionName', 'name', 'tier'],
            'defenderRecommendations': ['subscriptionId', 'subscriptionName', 'resourceGroup', 'resourceType', 'resourceName', 'category', 'recommendationSeverity', 'resourceId'],
            'inventory': ['subscriptionId', 'resourceGroup', 'location', 'resourceType', 'resourceName', 'resourceId'],
            'outOfScope': ['subscriptionId', 'resourceGroup', 'location', 'resourceType', 'resourceName', 'resourceId'],
            'resourceType': ['subscription', 'resourceType'],
            'advisor': ['subscriptionId', 'subscriptionName', 'resourceType', 'category', 'impact'],
            'impacted': ['source', 'category', 'impact', 'resourceType', 'recommendationId', 'subscriptionId', 'subscriptionName', 'resourceGroup', 'resourceName', 'resourceId'],
            'recommendations': ['recommendationId', 'subscriptionId', 'subscriptionName', 'resourceGroup', 'resourceName', 'resourceId', 'implemented', 'recommendationSource', 'azureServiceCategoryWellArchitectedArea', 'azureServiceWellArchitectedTopic', 'category', 'impact']
        };

        // Define dropdown fields per dataset
        const dropdownConfig = {
            'advisor': ['resourceType'],
            ...
            'defenderRecommendations': ['resourceType', 'category'],
        ...
        if (datasetFilters.includes(field)) {
            const uniqueValues = [...new Set(rows.map(r => r[field]).filter(v => v && v.trim()))].sort();
...
function updateRowCount() {
    ...
    if (!countDisplay) {
        ...
        countDisplay.className = 'text-muted small mb-2 d-flex justify-content-between align-items-center';
    ...
    countDisplay.innerHTML = `
        ...
        ${visibleRows.length !== allRows.length ? '<div class="text-warning"><i class="bi bi-funnel me-1"></i>Filtered</div>' : ''}
    `;
}
...
document.getElementById('apply-filters').addEventListener('click', () => navigate());
document.getElementById('clear-filters').addEventListener('click', () => {
    if (typeof clearAllTableFilters === 'function') {
...
function updateVisibleRowCount() {
    ...
    if (!countDisplay) {
        countDisplay = document.createElement('div');
        countDisplay.id = 'row-count';
        countDisplay.className = 'text-muted small mb-2';
        const tableCard = document.querySelector('.table-responsive').parentElement;
        tableCard.insertBefore(countDisplay, tableCard.firstChild);
    }

    countDisplay.innerHTML = `
        <i class="bi bi-table me-1"></i>
        Showing ${visibleRows.length} of ${allRows.length} rows
        ${visibleRows.length !== allRows.length ? '<span class="text-warning ms-2"><i class="bi bi-funnel me-1"></i>Filtered</span>' : ''}
    `;
}
