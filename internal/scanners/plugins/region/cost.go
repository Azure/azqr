...
func (s *RegionSelectorScanner) enrichWithCostData(ctx context.Context, cred azcore.TokenCredential, subscriptionID string, results []regionAvailability) {
	...
	targetRegions := make([]string, len(results))
	for i, r := range results {
		targetRegions[i] = r.region
	}

	// Query pricing for each meter across all target regions
	meterPricing := s.getMeterPricingAcrossRegions(ctx, meterMetadata, targetRegions)
	...
	for i := range results {
		region := strings.ToLower(results[i].region)
		// Skip pseudo-regions entirely
		...
		for meterID, pricing := range meterPricing {
			...
			if historicalCost == 0 {
				continue // Skip meters with no historical cost
			}

			// Find baseline price (original region price)
			var baselinePrice float64
			var baselineRegion string
			for _, mc := range meterCosts {
				if mc.meterID == meterID {
					baselineRegion = strings.ToLower(mc.armRegionName)
					// Skip pseudo-regions (global, europe) as they don't have meaningful pricing
					if baselineRegion == "global" || baselineRegion == "europe" {
						continue
					}
					if price, ok := pricing[baselineRegion]; ok && price > 0 {
						baselinePrice = price
						break
					}
				}
			...
			// Skip self-comparison (don't compare region with itself)
			if region == baselineRegion {
				continue
			}

			if regionPrice, exists := pricing[region]; exists && baselinePrice > 0 {
				// Calculate percentage difference
				priceDiff := ((regionPrice - baselinePrice) / baselinePrice) * 100

				// Weight by historical cost (this is the key difference from resource count weighting)
				weightedCostDiff += priceDiff * historicalCost
				totalWeight += historicalCost
				metersCompared++

				log.Debug().Msgf("Meter %s: Region %s vs %s: price %.4f vs %.4f = %.2f%% diff (weight: $%.2f)",
					meterID, region, baselineRegion, regionPrice, baselinePrice, priceDiff, historicalCost)
			}
		}

		if totalWeight > 0 {
			results[i].avgCostDifference = weightedCostDiff / totalWeight
			log.Debug().Msgf("Region %s: weighted avg cost difference %.2f%% (from %d meters, total cost weight $%.2f)",
				region, results[i].avgCostDifference, metersCompared, totalWeight)
		} else {
			log.Debug().Msgf("No pricing data available for region: %s", region)
		}
	}
