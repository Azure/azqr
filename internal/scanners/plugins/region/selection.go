...
func (s *RegionSelectorScanner) GetMetadata() plugins.PluginMetadata {
	return plugins.PluginMetadata{
		...
		ColumnMetadata: []plugins.ColumnMetadata{
			...
			{Name: "Current Resources", DataKey: "currentResources", FilterType: plugins.FilterTypeNone},
			{Name: "Available Resources", DataKey: "availableResources", FilterType: plugins.FilterTypeNone},
			{Name: "Unavailable Resources", DataKey: "unavailableResources", FilterType: plugins.FilterTypeNone},
...
func (s *RegionSelectorScanner) Scan(ctx context.Context, cred azcore.TokenCredential, subscriptions map[string]string, filters *models.Filters) (*plugins.ExternalPluginOutput, error) {
	...
	allResources, err := s.collectAllResources(ctx, cred, subscriptions, filters)
	if err != nil {
...
func (s *RegionSelectorScanner) getAllAzureRegions(ctx context.Context, cred azcore.TokenCredential, subscriptionID string) ([]string, error) {
	...
	// Add common Azure regions as fallback
	commonRegions := []string{
		...
		"swedencentral", "southeastasia", "eastasia", "australiaeast", "australiasoutheast",
		"japaneast", "japanwest", "koreacentral", "southindia", "centralindia", "westindia",
		"uaenorth", "southafricanorth", "qatarcentral", "polandcentral",
	}
	...
	for pager.More() {
		...
		for _, resource := range page.Value {
			if resource.Location != nil {
				...
				// Skip pseudo-regions that don't represent actual Azure regions
				if location != "global" && location != "europe" {
...
func (s *RegionSelectorScanner) calculateScores(results []regionAvailability) {
	for i := range results {
		// Score is weighted: 50% availability, 30% cost, 20% latency
		...
		results[i].score = (availabilityScore * 0.5) + (costScore * 0.3) + (latencyScore * 0.2)

		log.Debug().Msgf("Region %s scores: availability=%.2f (50%%), cost=%.2f (30%%), latency=%.2f (20%%), final=%.2f",
			results[i].region, availabilityScore, costScore, latencyScore, results[i].score)
	}
}

// generateOutputTable creates the output table from results
func (s *RegionSelectorScanner) generateOutputTable(results []regionAvailability) [][]string {
	table := [][]string{
		{"Subscription", "Target Region", "Current Resources", "Available Resources", "Unavailable Resources", "Availability %", "Avg Latency (ms)", "Avg Cost Difference %", "Recommendation Score", "Missing Resource Types"},
	}

	for _, result := range results {
		totalResources := result.availableTypes + result.unavailableTypes
...
func (s *RegionSelectorScanner) collectAllResources(ctx context.Context, cred azcore.TokenCredential, subscriptions map[string]string, filters *models.Filters) ([]*models.Resource, error) {
...
func (s *RegionSelectorScanner) buildInventoryForSubscription(subscriptionID string, allResources []*models.Resource) *resourceInventory {
	...
	log.Debug().Msgf("Subscription %s: Processed %d resources from inventory", renderers.MaskSubscriptionID(subscriptionID, true), resourceCount)
