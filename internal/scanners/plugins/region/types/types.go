...
// Resource types whose provider location list contains "global" are considered
// available in every region (e.g. microsoft.network/privatednszones,
// microsoft.cdn/profiles, microsoft.insights/activitylogalerts).
func (rtl *ResourceTypeLocationData) IsAvailable(resourceType, region string) bool {
	...
	// "global" in the location set means the resource type is available in every region.
	if _, isGlobal := locs["global"]; isGlobal {
		return true
	}
