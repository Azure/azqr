...
| where type =~ 'Microsoft.ResourceHealth/events' and properties.EventType == 'ServiceIssue' and properties.ImpactStartTime >= ago(90d)
| extend
...
func (s *AvailabilityScanner) Scan(ctx context.Context, cred azcore.TokenCredential, subscriptions map[string]string, filters *models.Filters) (*plugins.ExternalPluginOutput, error) {
	log.Info().Msg("Scanning service health across subscriptions")
	...
	for _, item := range result.Data {
		...
		// Filter excluded resource types
...
func init() {
	plugins.RegisterInternalPlugin("service-health", NewAvailabilityScanner())
}
