...
func TestStorageAnalyzer_Review(t *testing.T) {
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("StorageAnalyzer.Review() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("StorageAnalyzer.Review() = %v, want %v", got, tt.want)
			}
		})
	}
}
