...
// TestStorageAccountNoTagsViolation verifies that AZQR detects resources without tags (resources-001)
func TestStorageAccountNoTagsViolation(t *testing.T) {
	subscriptionID := helpers.RequireEnvVar(t, "AZURE_SUBSCRIPTION_ID")

	// Get path to Terraform fixture
	fixtureDir := helpers.GetFixturePath(t, "scenarios/Storage/storageAccounts/storage-no-tags")
